...
class employeeClass:
    def __init__(self, root):
        ...
        cmb_gender=ttk.Combobox(root,textvariable=self.var_gender,values=("Select","Male","Female","Other"),font=("goudy old style",15))
        cmb_gender.place(x=500,y=150,width=180)
        cmb_gender.current(0)
        # combobox = ttk.Combobox(root, values=["Option 1", "Option 2", "Option 3"])
        # combobox.pack()
        txt_contact = Entry(self.root,textvariable=self.var_conatct, font=("goudy old style", 15), bg="lightyellow").place(x=850, y=150,width=180)
        ...
        cmb_utype=ttk.Combobox(root,textvariable=self.var_utype,values=("Admin","Employee"),font=("goudy old style",15))
        ...
        self.EmployeeTable.heading("password", text="pass")
    ...
    def add(self):
        ...
        try:
            if self.var_emp_id.get() == "":
                messagebox.showerror("Error1","Employee ID Must be required",parent=self.root)
            else:
                cur.execute("Select * from employee where eid=?",(self.var_emp_id.get(),))
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error2","This Employee ID already assigned ,try different",parent=self.root)
                else:
    ...
    def update(self):
        ...
        try:
            if self.var_emp_id.get() == "":
                ...
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error2","Invalid Employee ID",parent=self.root)
                else:
